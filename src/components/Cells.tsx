...
import { DynamicMatrixGridContainerProps } from "../../typings/DynamicMatrixGridProps";
import { Header } from "./Header";

export function getCellValue(props: DynamicMatrixGridContainerProps, cell?: ObjectItem): ReactNode {
...
function cellTooltipValue(props: DynamicMatrixGridContainerProps, cell?: ObjectItem): string {
...
function rowTooltipValue(props: DynamicMatrixGridContainerProps, row?: ObjectItem): string {
...
function getRowHeaderValue(props: DynamicMatrixGridContainerProps, row?: ObjectItem): ReactNode {
...
interface CellsProps extends DynamicMatrixGridContainerProps {
    row: ObjectItem;
    rowIndex: number;
    loading: boolean;
    isHeader?: boolean;
    ...
    const cells =
        dataSourceColumn.items?.map(column => {
            ...
            if (pageCell && cell === undefined && !loading) {
                console.error(
                    `dynamic-matrix-grid widget - No cell found for row ${row.id} column ${column.id} while 'Optimize cell paging' is enabled.
